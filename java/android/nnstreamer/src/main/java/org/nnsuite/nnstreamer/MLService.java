...
public final class MLService implements AutoCloseable {
    ...
    private NewEventCallback mEventCallback;
     ...
     *
     * @see MLService#MLService(String, NewEventCallback)
     */
     ...
     * @param callback The function to be called when an event is received from machine-learning service.
     *
     ...
     */
    public MLService(String config, NewEventCallback callback) {
        if (!isValidString(config)) {
            throw new IllegalArgumentException("Given configuration is invalid.");
        }

        if (callback == null) {
            throw new IllegalArgumentException("Given callback is invalid.");
        }
    ...
    private void newDataReceived(String name, TensorsData data) {
        synchronized(this) {
            if (mEventCallback != null) {
                mEventCallback.onNewDataReceived(name, data);
            }
        }
    }
