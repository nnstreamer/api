...
/**
 * @brief Callback for tensor data stream of machine-learning inference.
 ...
 */
typedef int (*ml_data_cb) (const ml_tensors_data_h data, void *user_data);
...
typedef struct {
  ...
  ml_data_cb invoke_async_cb;    /**< Callback function to be called when the sub-plugin generates an output asynchronously. */
