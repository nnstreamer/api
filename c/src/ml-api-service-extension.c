    ...
    const ml_tensors_info_h info, void *user_data)
{
  ...
  _ml_service_get_callback_info (mls, &cb_info);

  if (cb_info.cb.new_data)
    cb_info.cb.new_data (mls, node_info->name, data, cb_info.pdata);
}
      ...
      switch (ext->type) {
        case ML_EXTENSION_TYPE_SINGLE:
        {
          ...
          if (status == ML_ERROR_NONE) {
            ...
            if (cb_info.cb.new_data)
              cb_info.cb.new_data (mls, msg->name, msg->output, cb_info.pdata);
          } else {
            _ml_error_report
...
_ml_extension_conf_parse_single (ml_service_s * mls, JsonObject * single)
{
  ...
  if (json_object_has_member (single, "key")) {
    ...
    status = _ml_service_conf_parse_path (file_node, &paths);
    if (status != ML_ERROR_NONE) {
      _ml_error_report
