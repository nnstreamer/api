...
typedef enum
{
  ML_OFFLOADING_SERVICE_TYPE_UNKNOWN = 0,
  ML_OFFLOADING_SERVICE_TYPE_MODEL_RAW,
  ML_OFFLOADING_SERVICE_TYPE_MODEL_URI,
  ML_OFFLOADING_SERVICE_TYPE_PIPELINE_RAW,
  ML_OFFLOADING_SERVICE_TYPE_PIPELINE_URI,

  ML_OFFLOADING_SERVICE_TYPE_MAX
} ml_offloading_service_type_e;
 ...
 */
static ml_offloading_service_type_e
_mlrs_get_service_type (gchar * service_str)
{
  ml_offloading_service_type_e service_type =
      ML_OFFLOADING_SERVICE_TYPE_UNKNOWN;

  if (!service_str)
    return service_type;

  if (g_ascii_strcasecmp (service_str, "model_raw") == 0) {
    service_type = ML_OFFLOADING_SERVICE_TYPE_MODEL_RAW;
  } else if (g_ascii_strcasecmp (service_str, "model_uri") == 0) {
    service_type = ML_OFFLOADING_SERVICE_TYPE_MODEL_URI;
  } else if (g_ascii_strcasecmp (service_str, "pipeline_raw") == 0) {
    service_type = ML_OFFLOADING_SERVICE_TYPE_PIPELINE_RAW;
  } else if (g_ascii_strcasecmp (service_str, "pipeline_uri") == 0) {
    service_type = ML_OFFLOADING_SERVICE_TYPE_PIPELINE_URI;
  } else {
 ...
 */
static gchar *
_mlrs_get_model_dir_path (_ml_offloading_service_s * offloading_s,
 ...
 */
static int
_mlrs_process_offloading_service (nns_edge_data_h data_h, void *user_data)
{
  void *data;
  nns_size_t data_len;
  ...
  switch (service_type) {
    case ML_OFFLOADING_SERVICE_TYPE_MODEL_URI:
    {
    ...
    case ML_OFFLOADING_SERVICE_TYPE_MODEL_RAW:
    {
    ...
    case ML_OFFLOADING_SERVICE_TYPE_PIPELINE_URI:
    {
      GByteArray *array = g_byte_array_new ();
    ...
    case ML_OFFLOADING_SERVICE_TYPE_PIPELINE_RAW:
  ...
  if (mls && event_type != ML_SERVICE_EVENT_UNKNOWN) {
    if (mls->cb_info.cb.event) {
      mls->cb_info.cb.event (mls, event_type, NULL, mls->cb_info.pdata);
    }
  }
...
_mlrs_edge_event_cb (nns_edge_event_h event_h, void *user_data)
{
  ...
  switch (event) {
    case NNS_EDGE_EVENT_NEW_DATA_RECEIVED:{
      ...
      ret = _mlrs_process_offloading_service (data_h, user_data);
      break;
    }
    default:
      break;
