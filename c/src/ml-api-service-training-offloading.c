 ...
 */
static void
_ml_training_offloading_node_info_free (gpointer data)
{
 ...
 */
int
ml_service_training_offloading_create (ml_service_s * mls, JsonObject * object)
{
  ...
  node = json_object_get_member (offloading_object, "training");
 ...
 */
static void
_ml_service_training_offloading_replce_pipeline_data_path (ml_service_s * mls)
{
  guint changed = 0;
  ml_training_services_s *training_s = NULL;
  ml_service_offloading_mode_e mode = ML_SERVICE_OFFLOADING_MODE_NONE;
...
_ml_service_training_offloading_node_info_new (ml_service_s * mls,
 ...
 */
static int
_ml_service_training_offloading_conf_parse_pipeline_node (ml_service_s * mls,
    JsonNode * node, ml_training_offloading_node_type_e type)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    node_info = _ml_service_training_offloading_node_info_new (mls, name, type);
    if (!node_info) {
      _ml_error_report_return (ML_ERROR_INVALID_PARAMETER,
 ...
 */
static int
_ml_service_training_offloading_conf_parse_pipeline (ml_service_s * mls,
    JsonObject * pipe)
{
  int ret = ML_ERROR_NONE;
  JsonNode *node;
 ...
 */
static int
_ml_service_training_offloading_ready_to_complete (ml_service_s * mls)
{
  int ret = ML_ERROR_NONE;
  int loop = 120;
  GList *list, *iter;
...
ml_service_training_offloading_stop (ml_service_s * mls)
{
  ...
  if (training_s->type == ML_TRAINING_OFFLOADING_TYPE_RECEIVER) {
    if (!g_file_test (training_s->trained_model_path, G_FILE_TEST_EXISTS)) {
      _ml_service_training_offloading_ready_to_complete (mls);
    }
  }
