...
_ml_service_destroy_internal (ml_service_s * mls)
{
  ...
  old_cb = mls->cb_info;
  memset (&mls->cb_info, 0, sizeof (ml_service_cb_info_s));
 ...
 */
void
_ml_service_get_callback_info (ml_service_s * mls,
 ...
 */
int
_ml_service_conf_parse_path (JsonNode * file_node, gchar ** path)
{
  guint i, n;

  *path = NULL;

  if (JSON_NODE_HOLDS_ARRAY (file_node)) {
    JsonArray *array = json_node_get_array (file_node);
    ...
    *path = g_string_free (val, FALSE);
  } else {
    *path = g_strdup (json_node_get_string (file_node));
  }

  return (*path != NULL) ? ML_ERROR_NONE : ML_ERROR_INVALID_PARAMETER;
}
 ...
 */
int
_ml_service_conf_parse_information (ml_service_s * mls, JsonObject * info)
{
  g_autoptr (GList) members = NULL;
  GList *iter;
  int status;

  members = json_object_get_members (info);
...
ml_service_new (const char *config, ml_service_h * handle)
{
  ...
  *handle = NULL;
  ...
  if (json_object_has_member (object, "information")) {
    ...
    status = _ml_service_conf_parse_information (mls, info);
    if (status != ML_ERROR_NONE)
      goto error;
  }
 ...
 */
int
ml_service_set_event_cb (ml_service_h handle, ml_service_callbacks_s * cb,
    void *user_data)
{
  ...
  /* Clear all if given callback is null. */
  if (cb) {
    mls->cb_info.cb = *cb;
