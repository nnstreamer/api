...
class MLOffloadingService : public ::testing::Test
{
  ...
  void SetUp () override
  {
    ...
    g_autofree gchar *sender_config = _get_config_path ("offloading_service_sender.conf");
    status = ml_service_new (sender_config, &client_h);
    ASSERT_EQ (status, ML_ERROR_NONE);

    g_autofree gchar *receiver_config
        = _get_config_path ("offloading_service_receiver.conf");
 ...
 */
static void
_offloading_service_cb_event (
    ml_service_h handle, int event_type, void *event_data, void *user_data)
{
  int status;
...
TEST_F (MLOffloadingService, createInvalidParam_n)
{
  ...
  status = ml_service_offloading_create (NULL);
  EXPECT_EQ (ML_ERROR_INVALID_PARAMETER, status);

  status = ml_service_offloading_create (service_h);
  EXPECT_EQ (ML_ERROR_NONE, status);

  status = ml_service_offloading_start (service_h, NULL);
  EXPECT_EQ (ML_ERROR_INVALID_PARAMETER, status);

  status = ml_service_offloading_start (NULL, option_h);
  EXPECT_EQ (ML_ERROR_INVALID_PARAMETER, status);
